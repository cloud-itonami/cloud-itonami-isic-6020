(ns tvbroadcastops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (BroadcastOpsAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the BroadcastLicenseGovernor
  ...
  human operator/broadcast coordinator. The approver resumes with
...
(defn build
  ...
  `tvbroadcastops.store/Store`).
  opts:
    :advisor      -- a `tvbroadcastops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; BroadcastOpsAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; BroadcastLicenseGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
            ...
            (case disposition
              ...
              {:disposition :escalate
               :audit [{:t :approval-requested
                        :op (:op request) :station-id (:station-id request)
                        :reason (or reason
          ...
          (if (= :approved (:status approval))
            {:disposition :commit
             ...
             :audit [{:t :approval-granted :op (:op request)
                      :station-id (:station-id request) :by (:by approval)}]}
            {:disposition :hold
